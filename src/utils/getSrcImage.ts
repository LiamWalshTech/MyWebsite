export async function getSrcImage(imagePath: string) {
  ...
  if (!hasMatchedImage) throw new Error(`"${imagePath}" does not exist in glob: '/src/images/**/*'`)

  const indexOfMatchedImage = Object.keys(images).findIndex(image => image === imagePath)
